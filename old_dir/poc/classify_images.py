...
tf.app.flags.DEFINE_string(
    'model_dir', '/content/AI-Cull-Duplicates/poc/model/imagenet',
...
def classify_image(run_at, list_data, csv_columns, top_x_features = None):

  # create the output/run_at folder if it does not exist
  if not os.path.exists('output/' + run_at + '/image_vectors'):
    # print(time_util.timestamp() + "[INFO] Missing output/run_at/nearest_neighbors folder, creating it...")
    os.makedirs('output/' + run_at + '/image_vectors')

  output_dir = 'output/' + run_at + '/image_vectors'
  #print(time_util.timestamp() + "[INFO] Building ANNOY Index...")
  # Build ANN Index from list_data
  return run_inference_on_images(list_data, output_dir, csv_columns)
...
def run_inference_on_images(list_data, output_dir, csv_columns):
  ...
  with tf.Session() as sess:
    ...
    for image_index, row in enumerate(list_data):
      image = row[csv_columns.index('Scaled JPG')]
      ...
      try:
        ...
        with tf.gfile.FastGFile(image, 'rb') as f:
          ...
          outfile_name = os.path.basename(image).replace('.jpg',".npz")
