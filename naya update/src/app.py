...
class create_session:
    ...
    def run_cluster_images(self, vector_matrix_path, cluster_output):
        ...
        write_list = []
        for i, cluster_indices in enumerate(optimized_clusters_indices):
            temp_list = []
            for index in cluster_indices:
                ...
                if cluster_output == 'one':
                    ...
                    print(
                        f"[ERROR] Wrong Choice, No such input for cluster_output: {cluster_output}")
                    print(f"[INFO] Use cluster_output with: \"all\" or \"one\"")
                    print(
                        f"[WARNING] Changing cluster_output to default \"all\" for now.")
                    cluster_output = 'all'
                    temp_list.append(temp_dict)
        ...
        keep_image_list = [True]*self.total
        ...
        for cluster_indices in optimized_clusters_indices:
            for index in cluster_indices[1:]:
                keep_image_list[index] = False
                # break

        self.images_dataframe[KEEP_IMAGE] = keep_image_list

    def save_csv(self, dataframe, csv_file_name, index = False):
        """
...
if __name__ == "__main__":
    ...
    new_session.preprocess(mode="hard overwrite")
